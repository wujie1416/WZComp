...
- (UIImage *)emptyBtnBackgroundImage
{
    /*
     ...
     NSDirectoryEnumerator *myDirectoryEnumerator=[[NSFileManager defaultManager] enumeratorAtPath:path];
     NSLog(@"用enumeratorAtPath:显示目录%@的内容：",path);
     while((path=[myDirectoryEnumerator nextObject])!=nil) NSLog(@"%@",path);
     */
    NSBundle *bundle = [NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:@"WZBaseClasses" ofType:@"bundle"]];
    UIImage *image = [UIImage imageNamed:@"blank_button" inBundle:bundle compatibleWithTraitCollection:nil];
    return _emptyBtnBackgroundImage ?: image;
}
